...
<body>
  <main class="page">
    ...
    <section class="grid">
      ...
      <section class="stack">
        ...
        <div class="card">
          <h2>安全策略</h2>
          <div class="row"><div class="field"><span>黑名单 IP（换行或逗号分隔）</span><textarea id="blockedIps" placeholder="203.0.113.10"></textarea></div><div class="field"><span>Origin / Referer 白名单域名</span><textarea id="allowedHosts" placeholder="dy.devforai.cn"></textarea></div></div>
          <div class="row"><div class="field"><span>要求浏览器来源头</span><select id="requireBrowserHeaders"><option value="false">关闭</option><option value="true">开启</option></select></div><div class="field"><span>拦截空 User-Agent</span><select id="blockEmptyUa"><option value="false">关闭</option><option value="true">开启</option></select></div></div>
          <div class="row"><div class="field"><span>快速加入黑名单 IP</span><input id="quickBlockIp" placeholder="输入要封禁的 IP" /></div><div class="field"><span>备注</span><input id="quickBlockReason" placeholder="例如：异常调用" /></div></div>
          <div class="row"><button id="saveSecurityBtn" class="btn primary">保存安全策略</button><button id="blockIpBtn" class="btn">加入黑名单</button></div>
          <div id="securityStatus" class="status muted">可配置 IP 黑名单和来源白名单，命中后写入安全审计。</div>
        </div>
        <div class="card">
          <h2>最近调用与安全审计</h2>
  ...
  <script>
    ...
    async function loadAll(){await Promise.allSettled([loadMetrics(),loadRateLimits(),loadSecurity(),loadLogs(),loadLlm(),loadPlans(),loadCodes()])}
    ...
    async function loadSecurity(){const d=await api('/api/admin/security');$('blockedIps').value=(d.blocked_ips||[]).join('\\n');$('allowedHosts').value=(d.allowed_origin_hosts||[]).join('\\n');$('requireBrowserHeaders').value=String(!!d.require_browser_headers);$('blockEmptyUa').value=String(!!d.block_empty_user_agent);set($('securityStatus'),'已读取安全策略'+(d.updated_at?'，更新时间 '+d.updated_at:'。'),'ok')}
    async function saveSecurity(){try{const body={blocked_ips:$('blockedIps').value,allowed_origin_hosts:$('allowedHosts').value,require_browser_headers:$('requireBrowserHeaders').value==='true',block_empty_user_agent:$('blockEmptyUa').value==='true'};await api('/api/admin/security',{method:'POST',body:JSON.stringify(body)});set($('securityStatus'),'安全策略已保存。','ok');loadLogs()}catch(e){set($('securityStatus'),e.message,'err')}}
    ...
    $('loginBtn').onclick=login;$('reloadBtn').onclick=loadAll;$('logoutBtn').onclick=()=>{token='';localStorage.removeItem('admin_token');set($('loginStatus'),'已清除本地令牌。')};$('testLlmBtn').onclick=()=>saveLlm(true);$('saveLlmBtn').onclick=saveLlm(false);$('saveRateBtn').onclick=saveRateLimits;$('saveSecurityBtn').onclick=saveSecurity;$('blockIpBtn').onclick=blockIp;$('savePlanBtn').onclick=savePlan;$('createCodeBtn').onclick=createCode;if(token)loadAll();
  </script>
</body>
</html>`;
}
