...
<head>
  ...
  <title>解析中控台 Admin</title>
  <link rel="icon" href="/favicon.svg" />
  <style>
    :root{color-scheme:dark;--bg:#050506;--panel:#111114;--soft:#19191e;--line:rgba(255,255,255,.10);--text:#f6f6f7;--muted:#9b9ba5;--cyan:#25f4ee;--pink:#fe2c55;--green:#56f39a;--yellow:#ffd166}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 12% 0,rgba(37,244,238,.12),transparent 28%),radial-gradient(circle at 90% 4%,rgba(254,44,85,.18),transparent 30%),#050506;color:var(--text);font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.page{max-width:1180px;margin:0 auto;padding:28px 16px 48px}.header{display:flex;justify-content:space-between;gap:14px;align-items:center;margin-bottom:18px}.brand{display:flex;gap:12px;align-items:center}.logo{width:44px;height:44px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(135deg,var(--cyan),var(--pink));font-weight:950;font-size:22px}h1{margin:0;font-size:26px}.sub{margin:4px 0 0;color:var(--muted);font-size:13px}.grid{display:grid;grid-template-columns:360px 1fr;gap:16px}.card{background:linear-gradient(180deg,rgba(255,255,255,.075),rgba(255,255,255,.035));border:1px solid var(--line);border-radius:24px;padding:18px;box-shadow:0 20px 70px rgba(0,0,0,.35)}h2{margin:0 0 14px;font-size:18px}.field{display:grid;gap:7px;margin:10px 0}.field span{font-size:12px;color:var(--muted)}input,select,textarea{width:100%;border:1px solid var(--line);border-radius:14px;background:#08080a;color:var(--text);padding:12px 13px;outline:none}textarea{min-height:92px;resize:vertical}.row{display:grid;grid-template-columns:1fr 1fr;gap:10px}.btn{border:0;border-radius:999px;background:#26262b;color:#fff;padding:12px 15px;font-weight:850;cursor:pointer}.primary{background:linear-gradient(90deg,var(--cyan),var(--pink));color:#fff}.ok{color:var(--green)}.err{color:var(--pink)}.muted{color:var(--muted)}.status{min-height:22px;font-size:13px;margin-top:10px;line-height:1.6}.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:10px}.metric{background:#08080a;border:1px solid var(--line);border-radius:18px;padding:14px}.metric b{display:block;font-size:22px}.metric span{font-size:12px;color:var(--muted)}.table{display:grid;gap:8px;max-height:320px;overflow:auto}.item{display:grid;grid-template-columns:1fr auto;gap:10px;background:#08080a;border:1px solid var(--line);border-radius:16px;padding:12px}.pill{border-radius:999px;background:rgba(37,244,238,.12);color:var(--cyan);padding:5px 9px;font-size:12px}.tools{display:flex;gap:8px;flex-wrap:wrap;margin:8px 0 0}@media(max-width:880px){.grid{grid-template-columns:1fr}.metrics{grid-template-columns:repeat(2,1fr)}.header{align-items:flex-start;flex-direction:column}.row{grid-template-columns:1fr}}
  </style>
</head>
<body>
  <main class="page">
    <header class="header"><div class="brand"><div class="logo">♪</div><div><h1>解析中控台</h1><p class="sub">轻量后台：模型配置、会员套餐、激活码、调用指标和审计。</p></div></div><a class="btn" href="/">返回前台</a></header>
    <section class="grid">
      <aside class="card">
        <h2>管理员登录</h2>
        ...
        <div id="loginStatus" class="status muted">登录后可保存模型配置和会员权益。</div>
      ...
      <section style="display:grid;gap:16px">
        ...
        <div class="card">
          <h2>最近调用与安全审计</h2>
          <p class="muted">展示接口调用、AI 调用、限流拦截、后台登录、套餐和激活码操作。</p>
  ...
  <script>
    ...
    async function loadAll(){await Promise.allSettled([loadMetrics(),loadLogs(),loadLlm(),loadPlans(),loadCodes()])}
    ...
    $('loginBtn').onclick=login;$('reloadBtn').onclick=loadAll;$('logoutBtn').onclick=()=>{token='';localStorage.removeItem('admin_token');set($('loginStatus'),'已清除本地令牌。')};$('testLlmBtn').onclick=()=>saveLlm(true);$('saveLlmBtn').onclick=()=>saveLlm(false);$('savePlanBtn').onclick=savePlan;$('createCodeBtn').onclick=createCode;if(token)loadAll();
  </script>
</body>
</html>`;
}
