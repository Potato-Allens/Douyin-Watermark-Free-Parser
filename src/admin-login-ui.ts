...
<body>
  ...
  <script>
    ...
    function friendly(message){const text=String(message||'\u8bf7\u6c42\u5931\u8d25');const rules=[[/admin credentials are invalid/i,'\u7ba1\u7406\u5458\u8d26\u53f7\u6216\u5bc6\u7801\u9519\u8bef'],[/admin password is not configured/i,'\u670d\u52a1\u5668\u672a\u914d\u7f6e\u7ba1\u7406\u5458\u5bc6\u7801'],[/admin totp code is invalid|totp code is invalid/i,'\u52a8\u6001\u7801\u9519\u8bef\uff0c\u8bf7\u8f93\u5165\u5f53\u524d 6 \u4f4d\u25968\u5b57'],[/totp code is required/i,'\u52a8\u6001\u7801\u5df2\u542f\u7528\uff0c\u91cd\u65b0\u663e\u793a\u4e8c\u7ef4\u7801\u524d\u9700\u8f93\u5165\u5f53\u524d 6 \u4f4d\u52a8\u6001\u7801'],[/temporarily locked/i,'\u767b\u5f55\u5931\u8d25\u6b21\u6570\u8fc7\u591a\uff0c\u8bf7\u7a0d\u540e\u518d\u8bd5'],[/request body is too large/i,'\u8bf7\u6c42\u5185\u5bb9\u8fc7\u5927'],[/content-type/i,'\u8bf7\u6c42\u683c\u5f0f\u9519\u8bef']];for(const rule of rules){if(rule[0].test(text))return rule[1]}return text}
    ...
    async function setupTotp(){busy(true);try{const auth=credentials();const data=await api('/api/admin/totp/bootstrap',{...auth,issuer:DEFAULT_ISSUER,account:auth.username||'admin'});pendingSecret=data.secret||'';$('totpSecret').value=data.secret||data.secret_masked||'';$('totpUri').value=data.otpauth_uri||'';$('totpQr').innerHTML=data.qr_svg||'<span>\u4e8c\u7ef4\u7801\u751f\u6210\u5931\u8d25</span>';set($('totpStatus'),data.enabled?'\u8eab\u4efd\u5df2\u6821\u9a8c\uff0c\u53ef\u91cd\u65b0\u626b\u7801\u540e\u767b\u5f55\u3002':'\u4e8c\u7ef4\u7801\u5df2\u751f\u6210\uff0c\u626b\u7801\u540e\u8f93\u5165 6 \u4f4d\u52a8\u6001\u7801\u3002','ok')}catch(error){set($('totpStatus'),error.message,'err')}finally{busy(false)}}
    async function enableTotp(){busy(true);try{const auth=credentials();await api('/api/admin/totp/bootstrap/verify',{...auth,secret:pendingSecret||$('totpSecret').value,code:auth.totp,issuer:DEFAULT_ISSUER,account:auth.username||'admin'});set($('totpStatus'),'\u52a8\u6001\u7801\u5df2\u542f\u7528\uff0c\u6b63\u5728\u8fdb\u5165\u540e\u53f0\u2026','ok');window.location.replace('/admin')}catch(error){set($('totpStatus'),error.message,'err')}finally{busy(false)}}
    $('loginForm').addEventListener('submit',login);$('setupTotpBtn').addEventListener('click',setupTotp);$('enableTotpBtn').addEventListener('click',enableTotp);
  </script>
</body>
</html>`;
}
